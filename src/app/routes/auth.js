...
export default new Hono()
  ...
  .post('/login/magic-link', async (c) => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        return c.redirect('/login/magic-link?error=' + encodeURIComponent(error.message || 'Failed to send magic link'));
      }

      console.log('Magic link sent successfully');
      return c.redirect('/login/magic-link?success=' + encodeURIComponent('Magic link sent! Check your email.'));
    } catch (error) {
      console.error('signInMagicLink catch error:', error);
      return c.redirect('/login/magic-link?error=' + encodeURIComponent('Network error'));
    }
  })
  .get('/magic-link', async (c) => {
    ...
    if (!token) {
      return c.redirect('/login?error=' + encodeURIComponent('Invalid magic link'));
    }
    ...
    try {
      ...
      if (!result.ok) {
        const error = await result.json();
        return c.redirect('/login?error=' + encodeURIComponent(error.message || 'Invalid or expired magic link'));
      }

      // Forward cookies from magic link verification
      forwardCookies(result, c);

      return c.redirect('/profile?success=' + encodeURIComponent('Successfully signed in!'));
    } catch (error) {
